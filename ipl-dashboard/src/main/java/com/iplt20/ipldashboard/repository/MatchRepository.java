...
public interface MatchRepository extends CrudRepository<MatchSummary,Long> {
	...
	/*List<MatchSummary> getByTeam1AndDateBetweenOrTeam2AndDateBetweenOrderByDateDesc(
		String teamName1,LocalDate startDate1,LocalDate endDate1,
		String teamName2,LocalDate startDate2,LocalDate endDate2
	);*/
	@Query("select m from MatchSummary m where (m.team1=:teamName or m.team2=:teamName) and m.date between :startDate and :endDate order by m.date desc")
	List<MatchSummary>getMatchesByTeamBetweenDates(@Param("teamName") String teamName,
